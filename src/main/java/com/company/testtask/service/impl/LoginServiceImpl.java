...
public class LoginServiceImpl implements LoginService {
    ...
    private final Trimmer<List<LoginDto>> loginTrimmer;
    private final Mapper<List<Login>, List<LoginDto>> loginMapper;
    private final MapperToResponseDto<List<LoginResponseDto>, List<LoginDto>> mapperToResponse;

    @Override
    public List<LoginResponseDto> findAll() {
        List<LoginDto> loginDtos = entityReader.createLoginDtos(PATH_TO_FILE);
        loginDtos = loginTrimmer.trimTabulationSymbol(loginDtos);
        return mapperToResponse.mapToResponseDto(loginDtos);
    }

    @Override
    public void saveInDb() {
        List<LoginDto> loginDtos = entityReader.createLoginDtos(PATH_TO_FILE);
        List<Login> logins = loginMapper.mapToEntity(loginDtos);
